#!/usr/bin/python
# SOURCE https://github.com/Grk0/python-libconf
# CHANGED FOR PROJECT by Normynator
...
class Tokenizer:
    ...
    token_map = compile_regexes([
        ...
        (IntToken, 'integer', r'([-+]?[0-9]+)(?!\w)'), # FIXED TO MATCH 1_ETC as name
        (BoolToken, 'boolean', r'(?i)(true|false)\b'),
        (StrToken, 'string', r'"([^"\\]|\\.)*"'),
        (Token, 'name', r'[A-Za-z0-9\*][-A-Za-z0-9_\*]*'),
        (Token, '}', r'\}'),
        (Token, '{', r'\{'),
        (Token, ')', r'\)'),
        (Token, '(', r'\('),
        ...
        while pos < len(string):
            ...
            for cls, type, regex in self.token_map:
                ...
                raise ConfigParseError(
                    "Couldn't load config in %r row %d, column %d: %r, %s, %d" %
                    (self.filename, self.row, self.column,
                     string[pos:pos + 20], m, pos))
